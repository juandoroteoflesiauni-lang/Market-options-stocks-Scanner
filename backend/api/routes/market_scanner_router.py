...
def _json_sanitize(obj: object) -> object:
    ...
    try:
        value = float(obj)  # type: ignore[arg-type]
...
async def get_market_scanner_universes() -> dict[str, dict[str, object]]:
...
@router.get("/presets")
async def get_market_scanner_presets() -> list[dict[str, object]]:
    """Return built-in scanner presets."""
    return [preset.model_dump(mode="json") for preset in list_market_scanner_presets()]


@router.get("/indicators")
async def get_market_scanner_indicators() -> dict[str, object]:
