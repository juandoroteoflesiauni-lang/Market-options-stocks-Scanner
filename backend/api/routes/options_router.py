...
from backend.quant_engine.engines.options.bsm import BlackScholesPricer, OptionType
...
from backend.quant_engine.engines.options.chain_institutional_analytics import (
    ChainInstitutionalAnalyticsResponse,
    build_chain_institutional_analytics,
)
from backend.quant_engine.engines.options.confluence_models import GEXLevels
from backend.quant_engine.engines.options.derivatives import GEXMath, VolatilitySurfaceMath
from backend.quant_engine.engines.options.gamma_flip_probability import (
    estimate_gamma_flip_probability,
)
from backend.quant_engine.engines.options.iv_primitives import (
    atm_iv_from_chain,
...
from backend.quant_engine.engines.options.options_confluence import OptionsConfluenceEngine
from backend.quant_engine.engines.options.options_flow_signal import OptionsFlowSignalEngine
from backend.quant_engine.engines.options.options_models import OptionsResult
...
def _build_gex_levels(
    strikes: np.ndarray[Any, Any],
    call_oi: np.ndarray[Any, Any],
    put_oi: np.ndarray[Any, Any],
    call_iv: np.ndarray[Any, Any],
    put_iv: np.ndarray[Any, Any],
    spot: float,
    tte: float,
    r: float = 0.04,
) -> GEXLevelsResponse:
...
def _build_iv_surface(
    ...
    spot: float,
    r: float = 0.04,
    underlying_closes: np.np.ndarray[Any, Any] | None = None,
