...
if __name__ == "__main__":
    ...
    logger.info("=" * 70)
    logger.info("  QuantumBeta — Skew_FatTails_Engine | Analysis Report")
    logger.info("=" * 70)
    logger.info(f"  Spot Price          : {result.spot_price}")
    logger.info(f"  ATM IV (ISD proxy)  : {result.atm_iv:.2%}")
    logger.info(f"  Put-Call IV Spread  : {result.put_call_iv_spread:.2%}")
    logger.info(f"  Implied Skewness λ1 : {result.implied_skewness:.4f}")
    logger.info(f"  Tail Risk Factor λ2 : {result.tail_risk_factor:.4f}")
    logger.info(f"  Composite Risk Score: {result.risk_score:.2f} / 100")
    logger.info("  ── RISK FLAG ──────────────────────────────────────")
    logger.info(f"  {result.risk_flag.value}")
    logger.info(f"  {result.flag_rationale}")
    logger.info("=" * 70)
    logger.info("\n  Per-Strike Skew Profile:")
    logger.info(result.skew_profile.to_string(index=False))
    ...
    logger.info(f"  {'Strike':>8}  {'BS Price':>10}  {'JR Price':>10}  {'Δ Price':>10}")
    for k in [430, 460, 490]:
        ...
        logger.info(f"  {k:>8}  {bs:>10.4f}  {jr:>10.4f}  {jr - bs:>+10.4f}")
    logger.info("=" * 70)
