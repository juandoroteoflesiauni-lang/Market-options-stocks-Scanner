...
class FundamentalValuation:
    ...
    def options_analysis(opts: OptionsChainSnapshot) -> OptionsAnalysisResult:
        ...
        # ── Put/Call ratio ────────────────────────────────────────────────────
        pcr_vol = opts.put_call_ratio_vol
        pcr_oi = opts.put_call_ratio_oi
    ...
    def fundamental_score(
        ...
        # ── Valuation component ───────────────────────────────────────────────
        val_score = 0.0
        if vm.pe_trailing is not None:
            val_score += _clamp(-0.5 * (vm.pe_trailing - 20.0) / 20.0, -0.5, 0.5)
        if vm.price_to_book is not None:
            val_score += _clamp(-0.3 * (vm.price_to_book - 3.0) / 3.0, -0.3, 0.3)
