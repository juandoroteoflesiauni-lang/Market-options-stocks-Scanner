...
class FeedbackCalibration:
    ...
    def calculate_model_error(
        self, history: list[dict[str, Any]], current_price: float
    ) -> dict[str, float]:
        """
        ...
        try:
            ...
            raw_data = cast(
                dict[str, Any],
                (
                    json.loads(raw_json_val)
                    if isinstance(raw_json_val, str)
                    else (raw_json_val if raw_json_val else {})
                ),
            )
    ...
    def adapt_parameters(
        self, base_params: dict[str, Any], feedback: dict[str, Any]
    ) -> dict[str, Any]:
        """
