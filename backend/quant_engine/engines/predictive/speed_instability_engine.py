...
def bs_delta(
    ...
    sigma: np.ndarray[Any, np.dtype[Any]],
    T: np.ndarray[Any, np.dtype[Any]],
    option_type: np.ndarray[Any, np.dtype[Any]],
) -> np.ndarray[Any, np.dtype[Any]]:
    d1 = _d1(S, K, r, sigma, T)
...
class SpeedInstabilityEngine:
    ...
    def instability_zones(self, n: int = 3) -> pd.DataFrame:
        ...
        assert self.df is not None
        agg = (
            self.df.groupby("strike")["net_swx"]
            .sum()
            .reset_index()
            .rename(columns={"net_swx": "total_net_swx"})
        )
    ...
    def speed_decay(
        ...
        strike: float,
        option_type: str = "C",
        t_range: tuple[float, float] = (0.001, 1.0),
        n_points: int = 120,
    ) -> pd.DataFrame:
        self._assert_computed()
    ...
    def _speed_profile_data(
        self, spot_range_pct: float = 0.15, n_points: int = 240
    ) -> pd.DataFrame:
        ...
        speed_by_strike = [
            {
                ...
                "call_speed": float(r.call_speed),
                "put_speed": float(r.put_speed),
            }
