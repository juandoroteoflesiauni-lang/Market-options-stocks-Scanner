...
class VolatilitySkewEngine:
    ...
    def _fit_curve(self) -> None:
        ...
        if self.fit_model == "polynomial":
            ...
            def sabr_wrapper(
                K: np.ndarray[Any, np.dtype[Any]], alpha: float, beta: float, rho: float, nu: float
            ) -> np.ndarray[Any, np.dtype[Any]]:
    ...
    def _predict_iv(
        self, strikes: np.ndarray[Any, np.dtype[Any]]
    ) -> np.ndarray[Any, np.dtype[Any]]:
        ...
        if self.fit_model == "sabr":
            assert self._fitted_params is not None
            alpha, beta, rho, nu = self._fitted_params
...
def build_skew_frames_from_portfolio(
    ...
    for K in sorted(df_long["strike"].unique()):
        ...
        if spot * 0.999 > K and not dfp.empty:
            curve_rows.append({"strike": K, "iv": float(dfp.iloc[0]["iv"])})
        elif spot * 1.001 < K and not dfc.empty:
...
def compute_volatility_skew_payload(
    ...
    try:
        ...
        for s in scenarios:
            if (
                s.get("iv_ratio") is not None
                and isinstance(s["iv_ratio"], float)
                and math.isnan(s["iv_ratio"])
            ):
                s["iv_ratio"] = None
        market_points = []
