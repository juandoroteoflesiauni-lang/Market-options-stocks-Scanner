...
class CalibratorBundle(BaseModel):
    ...
    def transform(
        self, raw_probs: np.ndarray[Any, np.dtype[np.float64]]
...
class EnsembleMetaLearnerPredictor:
    ...
    def __init__(
        self, model: Any, feature_names: list[str], calibrator: CalibratorBundle | None = None
    ) -> None:
        self.model_ = model
    ...
    def _predict_proba_raw(
        self, x_arr: np.ndarray[Any, np.dtype[np.float64]]
    ) -> np.ndarray[Any, np.dtype[np.float64]]:
    ...
    def predict_proba(self, x: dict[str, float] | np.ndarray[Any, Any]) -> Result[dict[str, float]]:
        ...
        try:
            ...
            if isinstance(x, dict):
                row = [
                    float(x.get(f, 0.0) if x.get(f) is not None else 0.0)
            ...
            if x_arr.shape[1] != len(self.feature_names):
                return Result.failure(
                    reason=f"Feature mismatch: expected {len(self.feature_names)}, got {x_arr.shape[1]}"
                )
