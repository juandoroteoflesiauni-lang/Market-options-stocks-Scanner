...
try:
    from backend.quant_engine.engines.options.options_models import ExposureRegime
except ImportError:
...
class StatisticalRiskCalculator:
    ...
    @staticmethod
    def var_95(returns: np.ndarray[Any, Any]) -> float:
        """Historical VaR 95% (positive value = loss magnitude)."""
        return float(abs(np.percentile(returns, 5)))

    @staticmethod
    def cvar_95(returns: np.ndarray[Any, Any]) -> float:
    ...
    @staticmethod
    def max_drawdown(equity_curve: np.ndarray[Any, Any]) -> float:
...
class HRPPortfolioOptimizer:
    ...
    @staticmethod
    def _quasi_diag(link: np.ndarray[Any, Any], n: int) -> list[int]:
