...
class OptionsEngine:
    ...
    def analyze_chain(
        ...
        if populate_higher_greeks:
            ...
            surface = GreekSurface(
                speed=np.nan_to_num(
                    BlackScholesPricer.speed_vec(spot, strikes, tte, r, mid_iv)
                ).tolist(),
                zomma=np.nan_to_num(
                    BlackScholesPricer.zomma_vec(spot, strikes, tte, r, mid_iv)
                ).tolist(),
                color=np.nan_to_num(
                    BlackScholesPricer.color_vec(spot, strikes, tte, r, mid_iv)
                ).tolist(),
                ultima=np.nan_to_num(
                    BlackScholesPricer.ultima_vec(spot, strikes, tte, r, mid_iv)
                ).tolist(),
            )
