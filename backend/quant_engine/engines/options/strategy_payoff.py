    ...
    def compute_payoff(
        ...
        for spot in spots:
            ...
            points.append(
                {
                    "spot": float(spot),
                    "pl": float(expiration_pl),
                    "expiration_pl": float(expiration_pl),
                    "theoretical_pl": float(theoretical_pl),
                    "iv_up_pl": float(iv_up_pl),
                    "iv_down_pl": float(iv_down_pl),
                    "time_shift_pl": float(time_shift_pl),
                }
            )
        ...
        return PayoffCurve(
            points=points,
            max_profit=max(pls) if pls else None,
    ...
    def _net_greeks(
        ...
        for leg in strategy.legs:
            ...
            totals["delta"] += (
                ...
                else self._computed_delta(leg, strategy, scenario)
            ) * sign_qty
            totals["gamma"] += (
                float(leg.gamma)
                if leg.gamma is not None
                else self._computed_gamma(leg, strategy, scenario)
            ) * sign_qty
            totals["theta"] += (
                float(leg.theta)
                if leg.theta is not None
                else self._computed_theta(leg, strategy, scenario)
            ) * sign_qty
            totals["vega"] += (
                float(leg.vega)
                if leg.vega is not None
                else self._computed_vega(leg, strategy, scenario)
            ) * sign_qty
        return totals
    ...
    def _break_evens(
        ...
        for left, right in zip(points, points[1:], strict=False):
            ...
            if l_pl * r_pl < 0:
                ...
                breaks.append(left["spot"] + ratio * (right["spot"] - left["spot"]))
