...
class GammaFlipEngine:
    ...
    def analyze_gamma_flip(
        ...
        if sigma <= 0.0:
            return Result.failure(reason=f"volatility sigma must be greater than zero. Got {sigma}")
        ...
        try:
            ...
            if len(sign_changes) > 0:
                ...
                try:
                    ...
                    flip_point = float(p_lo - g_lo * (p_hi - p_lo) / (g_hi - g_lo))
            ...
            open_interest_shocked[put_mask] = open_interest[put_mask] * (1.0 + shock_pct)
            ...
            sign_changes_shocked = np.where(np.diff(np.sign(gamma_profile_shocked)))[0]
            ...
            if len(sign_changes_shocked) > 0:
                ...
                try:
                    ...
                    flip_shocked = float(ps_lo - gs_lo * (ps_hi - ps_lo) / (gs_hi - gs_lo))
