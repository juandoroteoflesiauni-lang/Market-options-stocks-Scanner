...
async def get_analysis(symbol: str, interval: str = "5m") -> dict[str, Any]:
    ...
    if _has_canonical_analysis_builder(service):
        try:
            return await _maybe_await(
                service.build_analysis_snapshot(symbol, interval=interval)  # type: ignore[attr-defined]
            )
        except Exception as exc:
            detail = _error_detail(exc)
            logger.warning(
    ...
    try:
        klines = await service.fetch_klines_for_analysis(
            symbol,
            interval=interval,
            limit=2000,  # type: ignore[arg-type]
        )
    except Exception as exc:
        detail = _error_detail(exc)
        logger.warning(
    ...
    if is_equity_perp:
        ...
        if isinstance(ta_raw, BaseException):
            ...
            ta_result = ta_raw  # type: ignore[assignment]
        ...
        if isinstance(prob_raw, BaseException):
            ...
            prob_result = prob_raw  # type: ignore[assignment]
