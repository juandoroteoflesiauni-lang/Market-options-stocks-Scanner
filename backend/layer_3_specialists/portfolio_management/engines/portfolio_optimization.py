...
class PortfolioOptimizationEngine:
    ...
    @staticmethod
    def _safe_invert(cov: np.ndarray, n: int) -> tuple[np.ndarray, bool, str | None]:
        try:
    ...
    def _tangency_portfolio(
        inv_cov: np.ndarray, expected_returns: np.ndarray, rf: float, n: int, log: list[str]
    ...
    @staticmethod
    def _min_variance_portfolio(cov: np.ndarray, n: int, log: list[str]) -> tuple[np.ndarray, bool]:
        ones = np.ones((n, 1))
    ...
    @staticmethod
    def _corr_dist(V: np.ndarray) -> np.ndarray:
