...
DEFAULT_HISTORY_DB = (
    Path(__file__).resolve().parents[2] / "data" / "options_chain_analytics_history.sqlite3"
)
...
def _snapshot_from_response(
    ...
    zero_dte_share = max(
        (float(exp.zero_dte_gamma_share) for exp in response.expiry_analytics), default=0.0
    )
...
def _wall_alert(
    *,
    ...
    if (
        ...
        or spot <= 0
        or previous_level == current_level
    ):
        return None
...
def _temporal_alerts(
    previous: ChainAnalyticsSnapshot, current: ChainAnalyticsSnapshot
) -> list[ChainAlert]:
    ...
    if (
        zero_dte_delta >= ZERO_DTE_SURGE_MIN_POINTS
        and current.zero_dte_gamma_share >= ZERO_DTE_SURGE_ABSOLUTE_LEVEL
    ):
        alerts.append(
            ChainAlert(
...
class OptionsChainAnalyticsHistoryStore:
    ...
    def _connect(self) -> sqlite3.Connection:
        ...
        columns = {
            ...
            for row in conn.execute(
                "PRAGMA table_info(options_chain_analytics_snapshots)"
            ).fetchall()
        }
        if "standard_metrics_json" not in columns:
            conn.execute(
                "ALTER TABLE options_chain_analytics_snapshots ADD COLUMN standard_metrics_json TEXT NOT NULL DEFAULT '{}'"
            )
        conn.execute(
            """
    ...
    def latest_snapshot(
        self, symbol: str, expiry_scope: str | None
    ) -> ChainAnalyticsSnapshot | None:
