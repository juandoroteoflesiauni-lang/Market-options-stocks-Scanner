"""Motor de Análisis SMC (Smart Money Concepts) — Sector Técnico.

Implementa la detección vectorizada de BOS (Break of Structure), CHoCH (Change of Character),
Order Blocks institucionales, Fair Value Gaps y modelos ICT (Stop Hunt, OTE).
"""
...
# MIGRATION: Imports pendientes de matemáticas técnicas (serán migrados en fases posteriores)
# from ...layer_2_quant_engine.math_core import TechnicalMath

logger = logging.getLogger("quantum_analyzer.smc_engine")
...
# ─────────────────────────────────────────────────────────────────────────────
# §3  SMCEngine — Motor Analítico Puro
# ─────────────────────────────────────────────────────────────────────────────
...
class SMCEngine:
    ...
    def analyze(
        self, df: pd.DataFrame, ticker: str = "UNKNOWN", timeframe: str = "UNKNOWN"
    ) -> SMCResult:
        ...
        try:
            ...
            ict_models, dominant, agg_conf = self._detect_ict_models(
                df_work, order_blocks, fvg_zones, structure, sweeps
            )
            ...
            return SMCResult(
                ticker=ticker, timeframe=timeframe, sesgo=DirectionalBias.NEUTRAL, error=str(exc)
            )

    def _build_key_levels_approx(
        self, df: pd.DataFrame, obs: list[OrderBlock], fvgs: list[FairValueGap]
    ) -> dict[str, float]:
        ...
        if bull_obs:
            levels["ob_stop_loss"] = float(bull_obs[-1].low)
        bear_obs = [ob for ob in obs if ob.direction == "BEARISH"]
        if bear_obs:
            levels["ob_stop_loss_short"] = float(bear_obs[-1].high)
    ...
    def _validate_and_normalize(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not req.issubset(df_out.columns):
            raise ValueError(f"Missing columns: {req - set(df_out.columns)}")
        df_out.dropna(subset=list(req), inplace=True)
        if len(df_out) < _MIN_ROWS:
            raise ValueError(f"Insufficient data ({len(df_out)})")
    ...
    def _detect_order_blocks(self, df: pd.DataFrame) -> list[OrderBlock]:
        ...
        for t in range(n - 2):
            ...
            ar_t = ar[t]
            if np.isnan(ar_t) or ar_t < _EPSILON:
                continue
            ...
            ezone = l_t + (wick_l * 0.5 if r_wb > 1.0 else (h_t - l_t) * 0.5)
                ...
                if c_min_fwd[t1] >= ob_50:
                    obs.append(
                        OrderBlock(
                            ...
                            low=float(l_t),
                            close=float(c_t),
                            entry_zone=float(ezone),
                            delta=float(delta_eff),
                            r_wb=float(r_wb),
                            sweep_candle=bool(r_wb > 1.0),
                            fvg_present=bool(l[t2] > h_t),
                            strength=float(v[t] / vm[t]),
                            ob_50_level=float(ob_50),
                        )
                    )
    ...
    def _detect_bos_choch(self, df: pd.DataFrame, fvgs: list[FairValueGap]) -> list[StructureEvent]:
        ...
        for i in range(len(df)):
            if np.isnan(shs[i]) or np.isnan(sls[i]):
                continue
            c = df["close"].iloc[i]
            ...
            if c > shs[i] * bm:
                etype = (
                    StructureEventType.CHOCH_BULL
                    if sls[i] < sls.shift(lb)[i]
                    else StructureEventType.BOS_BULL
                )
                events.append(
                    StructureEvent(bar_index=i, event_type=etype.value, level=float(shs[i]))
                )
            elif c < sls[i] * (2.0 - bm):
                etype = (
                    StructureEventType.CHOCH_BEAR
                    if shs[i] > shs.shift(lb)[i]
                    else StructureEventType.BOS_BEAR
                )
                events.append(
                    StructureEvent(bar_index=i, event_type=etype.value, level=float(sls[i]))
                )
        return events
            ...
            if v[i] > vm[i] * self._liq_vol_factor:
                ...
                if h[i] > rh:
                    sweeps.append(
                        LiquiditySweep(
                            bar_index=i,
                            sweep_type=SweepType.BSL_SWEEP.value,
                            level=rh,
                            rvol=float(v[i] / vm[i]),
                        )
                    )
                    ...
                    sweeps.append(
                        LiquiditySweep(
                            bar_index=i,
                            sweep_type=SweepType.SSL_SWEEP.value,
                            level=rl,
                            rvol=float(v[i] / vm[i]),
                        )
                    )
    ...
    def _compute_bias(
        ...
        active_bull_obs = [
            ...
            if ob.direction == "BULLISH" and not getattr(ob, "mitigated", False)
        ]
        active_bear_obs = [
            ob
            for ob in order_blocks
            if ob.direction == "BEARISH" and not getattr(ob, "mitigated", False)
        ]

        active_bull_fvg = [
            f for f in fvg_list if f.direction == "BULLISH" and not getattr(f, "mitigated", False)
        ]
        active_bear_fvg = [
            f for f in fvg_list if f.direction == "BEARISH" and not getattr(f, "mitigated", False)
        ]

        recent_events = bos_choch_events[-10:] if bos_choch_events else []
        bull_events = [
            e for e in recent_events if "BULL" in str(getattr(e, "event_type", "")).upper()
        ]
        bear_events = [
            e for e in recent_events if "BEAR" in str(getattr(e, "event_type", "")).upper()
        ]
        ...
        bull_sweeps = [
            ...
            if getattr(s, "sweep_type", "").lower() in ("ssl_sweep", "equal_low_sweep")
        ]
        bear_sweeps = [
            s
            for s in recent_sweeps
            if getattr(s, "sweep_type", "").lower() in ("bsh_sweep", "equal_high_sweep")
        ]
    ...
    def _detect_ict_models(
    ...
    ) -> tuple[list[ICTModelResult], ICTModelResult | None, float]:
        ...
        if sweeps:
            ...
            if last in (SweepType.SSL_SWEEP.value, SweepType.BSL_SWEEP.value):
                models.append(
                    ICTModelResult(
                        name=ICTModelName.STOP_HUNT, confidence=_ICT_CONFIDENCE["STOP_HUNT"]
                    )
                )
    ...
    def _detect_ict_models_bearish_extension(self, df, order_blocks, sweeps, fvg_list) -> list:
        """
        ...
        active_bear_obs = [
            ob for ob in order_blocks if ob.direction == "bearish" and not ob.mitigated
        ]
    ...
    def _compute_ote_zone(
        self, structure: list[StructureEvent], df: pd.DataFrame
    ...
    def _smc_composite_score(
        ...
        if bias in (DirectionalBias.BULLISH, DirectionalBias.BULLISH_WATCH):
            ...
            active_bull_fvg = [
                f
                for f in fvg_list
                if f.direction == "BULLISH" and not getattr(f, "mitigated", False)
            ]
        ...
        if bias in (DirectionalBias.BEARISH, DirectionalBias.BEARISH_WATCH):
            ...
            active_bear_fvg = [
                f
                for f in fvg_list
                if f.direction == "BEARISH" and not getattr(f, "mitigated", False)
            ]
...
# Archivo      : smc.py
# Sub-capa     : Engine (Estructura Institucional)
# Eliminado    : Referencias QuantumBeta V1 / Alpha.
# Preservado   : Detección BOS/CHoCH validada FVG, δ >= 1.3 OB. Bias bidireccional (LONG/SHORT).
# Pendientes   : Reconexión real con TechnicalMath (VWAP/EMAs).
# ─────────────────────────────────────────────────────────
