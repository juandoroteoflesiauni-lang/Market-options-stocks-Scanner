...
def fit_gpd(returns: FloatArray, quantile: float = 0.95) -> Result[TailRisk]:
    ...
    try:
        ...
        if len(exceedances) < 5:
            ...
            return Result.success(TailRisk(
                shape=0.0,
                scale=0.0,
        ...
        cvar_99 = (var_99 + scale - shape * threshold) / (1 - shape) if shape < 1.0 else var_99 * 1.5
...
class ParticleFilter:
    ...
    def get_state(self) -> AdaptiveState:
        ...
        return AdaptiveState(
            pr_ordered=float(mean_state[0]),
            trend_strength=float(mean_state[1])
        )
...
def particle_filter_volatility(
    ...
    try:
        ...
        for r in returns:
            particles = np.maximum(1e-6, particles + kappa*(v_mean - particles)*dt +
...
def calculate_kelly_sizing(win_prob: float, payoff_ratio: float) -> Result[KellySizing]:
    ...
    try:
        loss_prob = 1.0 - win_prob
        full_kelly = (win_prob * payoff_ratio - loss_prob) / payoff_ratio if payoff_ratio > 0 else 0.0
        full_kelly = max(0.0, float(full_kelly))

        return Result.success(KellySizing(
