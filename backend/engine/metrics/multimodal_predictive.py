...
from backend.engine.metrics.matrix_ops import calculate_probabilistic_gex_gating
...
class _EventDrivenLSTMCell(_nn.Module):
    ...
    def __init__(
        ...
        self.x_proj = _nn.Conv2d(
            1, hidden_channels, kernel_size=kernel_size, padding=padding
        )
    ...
    def forward(
        ...
        base = (
            self.x_proj(x_2d)
            + self.h_proj(h_prev)
            + self.e_proj(e_t).unsqueeze(-1).unsqueeze(-1).expand(
...
class _EventDrivenLSTM(_nn.Module):
    def __init__(
        ...
        self.cells: _nn.ModuleList = _nn.ModuleList([
            _EventDrivenLSTMCell(input_height, input_width, hidden_channels, event_dim, kernel_size)
