    ...
    for i, k in enumerate(unique_strikes):
        ...
        if k < spot * 0.999:
            ...
            if np.any(put_mask):
                ...
                curve_ivs[i] = (
                    matching_ivs[0] if len(matching_ivs) > 0 else np.nan
                )
        elif k > spot * 1.001:
...
def predict_iv(
    ...
    if fit_model == "sabr":
        ...
        return sabr_approximation(
            strikes, forward, alpha, beta, rho, nu, t=tte
        )
...
def _get_iv_atm(
    ...
    if fitted_params is not None:
        ...
        return float(
            predict_iv(strikes, fit_model, fitted_params, spot, rate, tte)[0]
        )
...
class VolatilitySkewEngine:
    ...
    def analyze_volatility_skew(
        ...
        if spot <= 0.0:
            return Result.failure(
                reason=f"spot price must be greater than zero. Got {spot}"
            )
        if tte <= 0.0:
            return Result.failure(
        ...
        try:
            ...
            if fit_model_used == "sabr":
                ...
                def sabr_wrapper(
                    ...
                    return sabr_approximation(
                        k_arr, forward, alpha, beta, rho, nu, t=tte
                    )
                ...
                try:
                    ...
                    logger.warning(
                        "SABR fit failed to converge. Falling back to polynomial model."
                    )
            ...
            alert_message = ""
            history = (
                list(convexity_history) if convexity_history is not None else []
            )
            ...
            idx = np.linspace(
                0, len(strike_grid) - 1, num=min(100, len(strike_grid)), dtype=int
            )
            ...
            fitted_curve = [
                ...
                for k, iv in zip(
                    strike_grid.tolist(), iv_fitted.tolist(), strict=True
                )
            ]
