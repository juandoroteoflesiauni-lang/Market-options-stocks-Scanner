...
class VolatilityTermStructureEngine:
    ...
    def __init__(
        ...
        if self.short_tenor not in self.standard_tenors:
            logger.warning("short_tenor %d not in standard_tenors. It will be interpolated.", self.short_tenor)
        if self.long_tenor not in self.standard_tenors:
            logger.warning("long_tenor %d not in standard_tenors. It will be interpolated.", self.long_tenor)
    ...
    def analyze(
        ...
        try:
            ...
            if len(cleaned_curve) == 0:
                return Result.failure(reason="No valid points in atm_curve after applying filters (0.03 <= IV <= 2.00, DTE >= 0)")
            ...
            unique_ivs = np.bincount(inverse_indices, weights=cleaned_curve[:, 1]) / np.bincount(inverse_indices)
