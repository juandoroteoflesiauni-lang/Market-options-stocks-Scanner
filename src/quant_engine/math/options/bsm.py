...
def _iv_solver(
    ...
    def obj(s: float) -> float:
        return (_call_price(S, K, T, r, s) if opt == OptionType.CALL else _put_price(S, K, T, r, s)) - mkt_p
    ...
    try:
        ...
        if _IV_SOLVER_LO <= iv <= _IV_SOLVER_HI: return iv
    except Exception:
        pass
    try:
...
class BlackScholesPricer:
    ...
    @staticmethod
    def price(S: float, K: float, T: float, r: float, sigma: float, opt: OptionType = OptionType.CALL) -> float:
        T_s, s_s = max(T, _T_FLOOR), max(sigma, _SIGMA_FLOOR)
        return _call_price(S, K, T_s, r, s_s) if opt == OptionType.CALL else _put_price(S, K, T_s, r, s_s)

    @staticmethod
    def delta(S: float, K: float, T: float, r: float, sigma: float, opt: OptionType = OptionType.CALL) -> float:
        T_s, s_s = max(T, _T_FLOOR), max(sigma, _SIGMA_FLOOR)
    ...
    @staticmethod
    def theta(S: float, K: float, T: float, r: float, sigma: float, opt: OptionType = OptionType.CALL) -> float:
        T_s, s_s = max(T, _T_FLOOR), max(sigma, _SIGMA_FLOOR)
        return _theta(S, K, T_s, r, s_s, opt)

    @staticmethod
    def rho(S: float, K: float, T: float, r: float, sigma: float, opt: OptionType = OptionType.CALL) -> float:
        T_s, s_s = max(T, _T_FLOOR), max(sigma, _SIGMA_FLOOR)
    ...
    def greeks(
        ...
        d1 = _d1_scalar(S, K, T_s, r, s_s)
        res = {
            "theoretical_price": BlackScholesPricer.price(S, K, T_s, r, s_s, opt),
            "delta": _delta(S, K, T_s, r, s_s, opt),
            "gamma": _gamma(S, K, T_s, r, s_s),
            "vega":  _vega(S, K, T_s, r, s_s),
            "theta": _theta(S, K, T_s, r, s_s, opt),
            "rho":   _rho(S, K, T_s, r, s_s, opt),
            "d1": d1,
            "d2": d1 - s_s * math.sqrt(T_s)
        }
        if second_order:
            res["vanna"] = -_phi(d1) * (d1 - s_s * math.sqrt(T_s)) / s_s
            res["charm"] = _charm(S, K, T_s, r, s_s)
        return res
    ...
    def gamma_vec(
        ...
        return _gamma_vec_kernel(S, _as_float_array(K), _as_float_array(T), _as_float_array(sigma), r)
    ...
    def vega_vec(
        ...
        return _vega_vec_kernel(S, _as_float_array(K), _as_float_array(T), _as_float_array(sigma), r)
    ...
    def vanna_vec(
        ...
        return _vanna_vec_kernel(S, _as_float_array(K), _as_float_array(T), _as_float_array(sigma), r)
    ...
    def charm_vec(
        ...
        return _charm_vec_kernel(S, _as_float_array(K), _as_float_array(T), r, _as_float_array(sigma))
    ...
    def speed_vec(
        ...
        return _as_float_array((up - down) / max(2.0 * step, 1e-12))
    ...
    def zomma_vec(
        ...
        return _as_float_array((up - down) / np.maximum(2.0 * step, 1e-12))
