...
def _d1_vectorized(
    ...
    return (np.log(spot / strike) + (rate + 0.5 * sig_val**2) * t_val) / (
        sig_val * np.sqrt(t_val)
    )
...
def shadow_delta_position_multiplier(
    ...
    multiplier = float(
        max(1.0, min(max_leverage, multiplier * base_multiplier))
    )
...
class ShadowDeltaEngine:
    ...
    def analyze_shadow_delta(
        ...
        try:
            ...
            bs_deltas = bs_delta_vectorized(
                spot_price, strikes, tte_arr, rate_arr, ivs, is_calls
            )
            vannas = bs_vanna_vectorized(
                spot_price, strikes, tte_arr, rate_arr, ivs
            )
            ...
            net_bs_delta = float(
                np.sum(bs_deltas * quantities * self.contract_size)
            )
            net_shadow_delta = float(
                np.sum(shadow_deltas * quantities * self.contract_size)
            )
            ...
            post_shock_delta_errors = (
                post_shock_bs_deltas - post_shadow_deltas
            )
            ...
            for i in range(n):
                ...
                hedge_adj = (
                    -delta_gaps[i] * quantities[i] * self.contract_size
                )
