...
class TPOSkewnessEngine:
    ...
    def evaluate(self) -> TPOSkewnessSignal:
        ...
        return TPOSkewnessSignal(
            ...
            bracket_count=len(
                {b for level in self._levels.values() for b in level.brackets}
            ),
    ...
    def _ingest_range(self, low: float, high: float, bracket_id: int) -> None:
        ...
        step = max(1, int(np.ceil(bin_count / self.config.max_bins_per_bar))) if bin_count > self.config.max_bins_per_bar else 1
    ...
    def _touch_price_level(self, price: float, bracket_id: int) -> None:
        ...
        self._highest_price = price if self._highest_price is None else max(self._highest_price, price)
...
def _compact_levels(
    levels: dict[float, _MutableLevel], limit: int
) -> tuple[TPOLevel, ...]:
