...
from ...domain.technical.lob_models import OFIRegime, OFIResult, OFIAnalysisOutput  # noqa: F401
...
class OFIEngine:
    ...
    def update(self, snapshot: L1Snapshot) -> OFIResult:
        ...
        delta_bid = self.compute_delta_bid(
            prev.best_bid_price, prev.best_bid_size,
            snapshot.best_bid_price, snapshot.best_bid_size,
        )
        delta_ask = self.compute_delta_ask(
            prev.best_ask_price, prev.best_ask_size,
            snapshot.best_ask_price, snapshot.best_ask_size,
        )
    ...
    def compute_delta_bid(
        self,
        prev_bid_price: float, prev_bid_size: float,
        curr_bid_price: float, curr_bid_size: float,
    ) -> float:
    ...
    def compute_delta_ask(
        self,
        prev_ask_price: float, prev_ask_size: float,
        curr_ask_price: float, curr_ask_size: float,
    ) -> float:
    ...
    def _accumulate(self, raw_ofi: float, timestamp: float, volume_flux: float) -> tuple[float, int]:
