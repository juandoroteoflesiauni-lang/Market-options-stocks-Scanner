...
from ...math.technical.technical import shannon_entropy, atr
...
class SMCFractalEngine:
    ...
    def analyze(df_ohlcv: pd.DataFrame, ticker: str) -> FractalSignal:
        ...
        try:
            ...
            body     = np.abs(df_ohlcv["close"].to_numpy(dtype=float) - df_ohlcv["open"].to_numpy(dtype=float))
            displacement = 0.0
            ...
            bias = "CASH"
            if is_ordered:
                bull_ev = (fvg_active and fvg_direction == "BULLISH") or (is_institutional and impulse_bullish)
                bear_ev = (fvg_active and fvg_direction == "BEARISH") or (is_institutional and impulse_bearish)
            ...
            return FractalSignal(
                ...
                bias="CASH", fvg_size=0.0, entropy_score=4.0, is_fvg_active=False,
            )
