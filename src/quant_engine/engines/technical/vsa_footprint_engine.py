        ...
        try:
            if len(df_ohlcv) < 50:
                return VSAFootprintResult(
                    ticker=ticker, timestamp=ts, ok=False, error="Insufficient data"
                )
            ...
            return VSAFootprintResult(ticker=ticker, timestamp=ts, ok=False, error=str(exc))
