...
class VolumeNodeEngine:
    ...
    def compute_from_profile(
        ...
        *,
        ...
        if self.config.edge_bins_as_lvn:
            nodes.extend(
                self._infer_edge_lvns(ordered_prices, ordered_volumes, smoothed, nodes)
            )
    ...
    def compute_from_ohlcv(self, df: pd.DataFrame) -> VolumeNodeTopography:
        ...
        return self.compute_from_profile(prices, volumes, timestamp=timestamp, last_close=last_close)
    ...
    def _filter_by_prominence(
        ...
        for idx in hvn_candidates:
            ...
            if prominence >= threshold:
                nodes.append(TopographyNode(
                    price=float(prices[idx]), volume=float(volumes[idx]),
                    smoothed=peak, type="HVN", prominence=float(min(prominence, 1.0))
                ))
        ...
        for idx in lvn_candidates:
            ...
            if prominence >= threshold:
                nodes.append(TopographyNode(
                    price=float(prices[idx]), volume=float(volumes[idx]),
                    smoothed=valley, type="LVN", prominence=float(min(prominence, 1.0))
                ))
        return nodes
    ...
    def _infer_edge_lvns(
        ...
        for idx in (0, len(prices) - 1):
            ...
            if price in existing_prices:
                continue
            edges.append(TopographyNode(
                price=price, volume=float(volumes[idx]),
                smoothed=float(smoothed[idx]), type="LVN",
