...
class VPOCMigrationEngine:
    ...
    def build_rolling_signal(
        ...
        chunks = [
            frame.iloc[s : s + chunk_size]
            for s in range(0, len(frame), chunk_size)
        ][:size]
        profiles = [
            self.build_profile_from_bars(c.copy(), f"window-{i + 1}")
    ...
    def _resolve_tick_size(self, price_min: float, price_max: float) -> float:
        ...
        if bin_count > self.config.max_bins:
            tick_size = max(
                (price_max - price_min) / max(self.config.max_bins - 1, 1), tick_size
            )
        return max(float(tick_size), 1e-9)

    def _bins_from_bars(
        self, frame: pd.DataFrame, tick_size: float
        ...
        for row in frame.itertuples(index=False):
            ...
            for i in range(levels):
                ...
                bd = bins.setdefault(price, {"total_volume": 0.0, "bid_volume": 0.0, "ask_volume": 0.0})
    ...
    def _calculate_value_area(
        ...
        while accum < target:
            ...
            if not can_down or (can_up and up_vol >= down_vol):
                ...
                hi_idx += 1
                if accum < target and hi_idx < len(ladder) and up_vol > ladder[hi_idx - 1].total_volume:
                    accum  += ladder[hi_idx].total_volume
                    hi_idx += 1
            else:
