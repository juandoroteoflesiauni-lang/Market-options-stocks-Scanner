...
class VWAPEngine:
    ...
    def _compute_bands(self, vwap: float, sd: float) -> VWAPBands:
        ...
        return VWAPBands(
            upper3=round(vwap + m3 * sd, p), upper2=round(vwap + m2 * sd, p),
            upper1=round(vwap + m1 * sd, p), lower1=round(vwap - m1 * sd, p),
            lower2=round(vwap - m2 * sd, p), lower3=round(vwap - m3 * sd, p),
        )
    ...
    def _is_valid_point(point: PricePoint) -> bool:
        return (
            point.price > 0 and point.volume > 0
            and isfinite(point.price) and isfinite(point.volume)
        )
