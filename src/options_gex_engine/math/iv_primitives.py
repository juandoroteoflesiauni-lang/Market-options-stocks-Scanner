...
def iv_percentile(iv: float, historical_ivs: FloatArray) -> float:
    ...
    if not math.isfinite(iv) or len(historical_ivs) == 0:
        return np.nan
    hist_arr = np.asarray(historical_ivs, dtype=np.float64)
    valid = hist_arr[np.isfinite(hist_arr)]
    if len(valid) == 0:
        return np.nan
...
def compute_term_structure(atm_ivs: FloatArray, dtes: FloatArray) -> dict[str, Any]:
    ...
    res = {
        ...
        "front_iv": np.nan, "back_iv": np.nan, "kink_dte": np.nan,
        "contango": False, "backwardation": False,
    }

    n = len(s_v)
    if n < 2:
        if n == 1: res["front_iv"] = res["back_iv"] = float(s_v[0])
        return res
    ...
    res.update({
        ...
        "front_iv": float(s_v[ord_idx[0]]), "back_iv": float(s_v[ord_idx[-1]]),
        "contango": slope > _CONTANGO_MIN, "backwardation": slope < -_CONTANGO_MIN,
    })
...
def atm_iv_from_chain(strikes: FloatArray, call_ivs: FloatArray, put_ivs: FloatArray, spot: float) -> float:
    ...
    if math.isfinite(c) and math.isfinite(p): return float((c + p) / 2.0)
    if math.isfinite(c): return float(c)
    if math.isfinite(p): return float(p)
