...
class VolatilitySurfaceMath:
    ...
    def bl_pdf(params: SVIParameters, r: float = 0.04, n_points: int = 500) -> ImpliedPDFResult:
        ...
        left_tail = _trapz1d(_as_float_array(density[K_grid < 0.9 * F]), _as_float_array(K_grid[K_grid < 0.9 * F]))
        right_tail = _trapz1d(_as_float_array(density[K_grid > 1.1 * F]), _as_float_array(K_grid[K_grid > 1.1 * F]))
        ...
        return ImpliedPDFResult(
            ...
            forward=F,
            tte=T,
            risk_neutral_mean=round(mean_q, 6),
            risk_neutral_std=round(std_q, 2),
            skewness=round(skew_q, 4),
            excess_kurtosis=round(kurt_q, 4),
            left_tail_prob=round(left_tail, 4),
            right_tail_prob=round(right_tail, 4),
            tail_regime=reg,
        )
...
class GEXMath:
    ...
    def vanna_cex_exposure(
        ...
        return round(vex, 2), round(cex, 4)
...
# Archivo      : derivatives.py
# Sub-capa     : Engine (Surface & Exposure)
# Eliminado    : Lógica BSM genérica (duplicada en bsm.py), SMC Confluence, Risk Calc.
# Preservado   : SVI, Breeden-Litzenberger, GEX/VEX/CEX Exposure logic.
# Pendientes   : Mover Risk Calculator a modulo separado si es necesario.
# ─────────────────────────────────────────────────────────────
