...
class ExpectedMoveEngine:
    ...
    def _validate_inputs(spot: float, iv: float, dte: int) -> None:
        ...
        if spot <= 0:
            raise ValueError(
                f"[ExpectedMoveEngine] 'spot' must be strictly positive "
                f"(received {spot})."
            )
        if not (0 < iv <= 10.0):
            raise ValueError(
        ...
        if dte <= 0:
            raise ValueError(
                f"[ExpectedMoveEngine] 'dte' must be a positive integer "
                f"(received {dte})."
            )
    ...
    def calculate(
        ...
        if round_prices and tick_size is None:
            raise ValueError(
                "[ExpectedMoveEngine] 'tick_size' must be provided "
                "when round_prices=True."
            )
...
if __name__ == "__main__":
    ...
    r2 = ExpectedMoveEngine.calculate(
        spot=150.00, iv=0.30, dte=30,
        round_prices=True, tick_size=0.01,
    )
    ...
    print(SEPARATOR)
    entry_zone: float = ExpectedMoveEngine.get_lower_bound(
        spot=4500.00, iv=0.18, dte=45
    )
