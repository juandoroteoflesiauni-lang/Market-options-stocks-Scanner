...
class ShadowDeltaEngine:
    ...
    def _validate_and_prepare(self) -> None:
        ...
        self.df["bs_delta"] = self.df.apply(
            lambda row: bs_delta(
                self.spot, row["strike"], row["expiry"],
                row["r"], row["iv"], row["option_type"]
            ), axis=1
        )
    ...
    def _compute_skew_slope(self) -> None:
        ...
        if self.regularize:
            ...
            skew_slopes = cast(np.ndarray[Any, np.dtype[Any]], np.clip(skew_slopes, -self.skew_cap, self.skew_cap))
    ...
    def _compute_shadow_delta(self) -> None:
        ...
        """
        self.df["shadow_delta"] = (
            self.df["bs_delta"] + self.df["vanna"] * self.df["skew_slope"]
        )
    ...
    def portfolio_summary(self) -> pd.DataFrame:
        ...
        """
        cols = [
            "strike", "option_type", "iv", "expiry",
            "bs_delta", "shadow_delta", "delta_gap", "delta_gap_pct",
            "vanna", "skew_slope", "open_interest", "quantity"
        ]
    ...
    def stress_test(self, shock_pct: float = -0.05) -> pd.DataFrame:
        ...
        for _, row in self.df.iterrows():
            ...
            delta_shocked_naive = bs_delta(
                shocked_spot, row["strike"], row["expiry"],
                row["r"], row["iv"], row["option_type"]
            )
            ...
            delta_shocked_adjusted = bs_delta(
                shocked_spot, row["strike"], row["expiry"],
                row["r"], iv_adjusted, row["option_type"]
            ...
            rows.append({
                ...
                "pct_error"           : (
                    ...
                    / max(abs(delta_shocked_adjusted), 1e-6) * 100
                ),
            })
    ...
    def plot_delta_curves(
        ...
        for row_n in [1, 2, 3]:
            fig.add_vline(
                x=self.spot, line_dash="dot", line_color="#f59e0b",
        ...
        for typ_df, label in [(calls, "CALL"), (puts, "PUT")]:
            ...
            oi_size = (
                ...
                if show_oi_bubble else 8
            )

            # BS Delta
            fig.add_trace(go.Scatter(
                x=typ_df["strike"], y=typ_df["bs_delta"],
                mode="lines+markers",
                name=f"Δ_BS ({label})",
                ...
                hovertemplate=(
                    "<b>Strike</b>: %{x:.1f}<br>"
                    "<b>Δ_BS</b>: %{y:.4f}<extra></extra>"
                ),
            ), row=1, col=1)

            # Shadow Delta
            fig.add_trace(go.Scatter(
                x=typ_df["strike"], y=typ_df["shadow_delta"],
                mode="lines+markers",
        ...
        fig.update_xaxes(
            title_text="Strike Price", row=3, col=1,
    ...
    def print_report(self) -> None:
        ...
        top5 = (summary.nlargest(5, "delta_gap_pct")[
            ["strike", "option_type", "bs_delta", "shadow_delta",
             "delta_gap_pct", "hedge_adj_shares"]
        ])
...
def build_demo_portfolio(spot: float = 5000.0) -> pd.DataFrame:
    ...
    oi = (5000 * np.exp(-0.5 * ((strikes - spot) / 150)**2) +
          np.random.randint(100, 500, n)).astype(int)
