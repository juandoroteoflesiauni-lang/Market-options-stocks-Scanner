...
_CRISIS_KEYWORDS: Final[list[str]] = [
    "boicot", "escandalo", "fraude", "estafa", "verguenza",
    "despido", "demanda", "ilegal", "cancelado", "corrupcion",
]

_SARCASM_REGEX: Final[re.Pattern[str]] = re.compile("|".join(_SARCASM_PATTERNS), flags=re.IGNORECASE)
_CRISIS_KW_REGEX: Final[re.Pattern[str]] = re.compile(r"\b(?:" + "|".join(_CRISIS_KEYWORDS) + r")\b", flags=re.IGNORECASE)
...
class SentimentEngine:
    ...
    BULLISH_KEYWORDS = {
        "upgrade": 0.5, "beating": 0.4, "outperform": 0.3, "growth": 0.2,
        "dovish": 0.6, "easing": 0.4, "buyback": 0.3, "breakout": 0.4
    }

    BEARISH_KEYWORDS = {
        "downgrade": 0.5, "missing": 0.4, "underperform": 0.3, "decline": 0.2,
        "hawkish": 0.6, "tightening": 0.4, "recession": 0.5, "breakdown": 0.4
    }

    @staticmethod
    def analyze(
        news_summaries: list[str],
        external_indicators: dict[str, Any] | None = None
    ) -> SentimentResult:
        ...
        if news_count > 0:
            ...
            for news in news_summaries:
                ...
                for word, weight in SentimentEngine.BULLISH_KEYWORDS.items():
                    if word in content: s += weight; themes.add(word)
        ...
        consensus = "BULLISH" if final_score >= 0.25 else "BEARISH" if final_score <= -0.25 else "NEUTRAL"
...
class SentimentReputationEngine:
    ...
    def analyze_dataframe(df: pd.DataFrame) -> pd.DataFrame | None:
        ...
        try:
            res_df = df.copy()
            res_df["_clean"] = res_df["text"].astype(str).str.lower().map(lambda x: _WHITESPACE_REGEX.sub(" ", x))
    ...
    def summarize(df: pd.DataFrame) -> SentimentResult:
        ...
        if len(df) == 0: return SentimentResult()

        avg_score = float(df["sentiment_score"].mean())
        consensus = "BULLISH" if avg_score >= 0.05 else "BEARISH" if avg_score <= -0.05 else "NEUTRAL"

        themes = []
        if df["crisis_detected"].any(): themes.append("crisis_keyword")
        if df["sarcasm_detected"].any(): themes.append("sarcastic_buzz")
