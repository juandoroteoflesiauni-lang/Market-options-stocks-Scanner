...
class VolatilityTermStructureEngine:
    ...
    def build_term_structure(self) -> "VolatilityTermStructureEngine":
        ...
        for snap_date, group in self._raw_chain.groupby("snapshot_date"):
            ...
            if len(dte_obs) >= 4:
                ...
                cs = CubicSpline(dte_obs, iv_obs, bc_type="not-a-knot",
                                 extrapolate=True)
                interp_fn = cs
            else:
    ...
    def compute_metrics(self) -> "VolatilityTermStructureEngine":
        ...
        available = [t for t in self.standard_tenors
                     if self.short_tenor < t < self.long_tenor]
        ...
        ts["slope"]      = (ts["iv_long"] - ts["iv_short"]) / (
                             self.long_tenor - self.short_tenor)
        ts["curvature"]  = ts[m_col] - (ts["iv_short"] + ts["iv_long"]) / 2

        # ── Z-Score rolling de la pendiente ─────────────────
        ts["slope_roll_mean"] = ts["slope"].rolling(
            self.zscore_window, min_periods=5).mean()
        ts["slope_roll_std"]  = ts["slope"].rolling(
            self.zscore_window, min_periods=5).std()
        ts["slope_zscore"]    = (
            (ts["slope"] - ts["slope_roll_mean"]) /
            ts["slope_roll_std"].replace(0, np.nan)
        )
