...
class OptionsMarketAnalyzer:
    ...
    def _engineer_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if nan_mask.any():
            ...
            df["previous_open_interest"] = df["previous_open_interest"].fillna(
                df["open_interest"]
            )
    ...
    def _compute_thresholds(self, df: pd.DataFrame) -> None:
        ...
        self._thresholds = {
            ...
            "oi_inc_min": np.nanpercentile(
                active.loc[active["net_oi_change"] > 0, "net_oi_change"],
                cfg.oi_increase_percentile,
            ) if (active["net_oi_change"] > 0).any() else 1,
            "oi_dec_min": np.nanpercentile(
    ...
    def _classify_row(self, row: pd.Series) -> str:
        ...
        is_flat_oi      = (
            pd.notna(ratio) and abs(ratio) < cfg.flat_oi_ratio_threshold
        )
    ...
    def get_thresholds(self) -> dict[str, Any]:
        ...
        if not self._thresholds:
            raise RuntimeError(
                "No thresholds computed yet. "
                "Call analyze_volume_oi_dynamics() first."
            )
    ...
    def summary(self, result_df: pd.DataFrame) -> pd.DataFrame:
        ...
        return (
            result_df
            .groupby(["signal_type", "option_type"])
            .size()
...
def _get_uoa_strikes(df: pd.DataFrame, spot: float) -> list[dict[str, Any]]:
    ...
    for _, row in top.iterrows():
        price_est = float(row["last_price"]) if "last_price" in row and pd.notna(row.get("last_price")) else None
        records.append({
...
if __name__ == "__main__":
    ...
    output_cols = [
        ...
        "volume", "net_oi_change", "volume_oi_ratio", "signal_type",
    ]
    ...
    logger.info(
        ...
        analyzer
        .top_signals(result, Signal.NEW_POSITION, n=5)[output_cols]
