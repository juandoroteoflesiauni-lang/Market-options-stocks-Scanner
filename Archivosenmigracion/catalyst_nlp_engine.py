...
_BULLISH_SIGNALS: list[str] = [
    "record", "accelerat", "exceed", "outperform", "strong demand", "margin expansion",
    "guidance raised", "ahead of expectations", "robust growth", "significant opportunity",
    "record revenue", "record earnings", "strong pipeline", "beat", "upside",
    "confident", "momentum", "ahead of plan", "double-digit growth", "new highs",
]

# Words that signal caution, uncertainty, or deteriorating conditions
_BEARISH_SIGNALS: list[str] = [
    "headwind", "challeng", "uncertain", "slowing", "below expectations",
    "guidance lowered", "miss", "disappoint", "pressure", "margin compression",
    "macro concerns", "cautious", "difficult environment", "softening", "decelerat",
    "impairment", "write-down", "restructuring", "layoff", "cost cutting",
    "weaker than", "softer", "reduce guidance",
]

# Words that signal elevated tail risk or systemic stress
_ALARMING_SIGNALS: list[str] = [
    "going concern", "liquidity crisis", "debt covenant", "default risk",
    "regulatory investigation", "class action", "material weakness",
    "restatement", "auditor concern", "systemic risk", "force majeure",
    "insolvency", "bankruptcy", "fraud", "SEC investigation",
]
...
class CatalystNLPEngine:
    ...
    async def analyze(self,
        ...
        yesterday_str = (today - timedelta(days=5)).strftime("%Y-%m-%d") # for rates if needed elsewhere
        ...
        results = {name: res for name, res in zip(task_names, task_results) if not isinstance(res, Exception)}
        ...
        if transcript_list:
            try:
                ...
                if transcript and transcript.content:
                    ...
                    for s in sentences:
                        s_l = s.lower()
                        if any(kw in s_l for kw in _BULLISH_SIGNALS + _BEARISH_SIGNALS + _ALARMING_SIGNALS):
        ...
        composite_bear = (
            self.TRANSCRIPT_WEIGHT * bear_ratio_t
            + self.NEWS_WEIGHT * bear_ratio_n
        )
