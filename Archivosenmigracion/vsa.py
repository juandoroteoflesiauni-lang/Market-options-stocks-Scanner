"""Motor de Análisis VSA (Volume Spread Analysis) — Sector Técnico.
...
# MIGRATION: Imports pendientes de motores especializados (fijados en Fase 3)
from ..ia_probabilistico.engines.vsa_forecast_engine import VSAForecastEngine
...
# ─────────────────────────────────────────────────────────────────────────────
# §4  VSAEngine — Motor Analítico Puro
# ─────────────────────────────────────────────────────────────────────────────
...
class VSAEngine:
    ...
    def __init__(self, config: VSAConfig | None = None) -> None:
        ...
        # MIGRATION: Sub-engines inicializados en Fase 3
    ...
    def analyze(
        ...
        try:
            ...
            # Resultados impulsados por el nuevo especialista IA
            vfi_res = self.forecast_eng.calculate_vfi(df)
            vfi_val, vfi_slope = vfi_res["vfi"], vfi_res["slope"]
            is_forecast_climax = self.forecast_eng.generate_vp_forecast(df)
            fs_support, fs_resistance = self.forecast_eng.detect_footprint_clusters(df)
    ...
    def compute_adv_last(self, df: pd.DataFrame) -> float:
        last = df.iloc[-1]
        # Simplificación de ADV para evitar dependencia externa en migración
...
# ─────────────────────────────────────────────────────────────────────────────
# §5  BLOCKS — PRIVATE IMPLEMENTATION
# ─────────────────────────────────────────────────────────────────────────────
...
def _consolidate_signal(
    ...
    """Consolida labels VSA recientes en sesgo direccional simetrico."""
...
def _evaluate_short_signal_0dte(df: pd.DataFrame, mfi_col: str) -> bool:
    """Senal short intraday por distribucion: espejo operativo del long 0DTE."""
    if mfi_col not in df.columns or len(df) < 3:
        return False
...
def _vsa_composite_score(
    ...
    """Score VSA simetrico: positivo long, negativo short."""
...
# Archivo      : vsa.py
# Sub-capa     : Engine (Lógica de Señal Pura)
# Eliminado    : Referencias QuantumBeta, bloques redundantes en scoring.
# Preservado   : Clasificación de 6 etiquetas, Absorción (A_index), Weis Wave.
# Pendientes   : Re-conexión con VFI, Forecast y Footprint sub-engines.
# ─────────────────────────────────────────────────────────
