...
class OptionFlowEvent(_SafeBaseModel):
    ...
    @classmethod
    def from_row(cls: type[OptionFlowEvent], row: dict[str, Any]) -> OptionFlowEvent:
        ...
        side = cls._normalize_side(row.get("side", row.get("aggressor", row.get("condition", "unknown"))))
...
class OptionsFlowSignal(_SafeBaseModel):
...
class OptionsFlowSignalEngine:
    ...
    def analyze(
        ...
        if call_vol + put_vol <= 0 or call_oi + put_oi <= 0:
            limitations.append("Volume or open interest unavailable; flow signal is not actionable.")
        unknown_sides = sum(1 for event in events if event.side == "unknown" and event.volume > 0)
        if unknown_sides:
            limitations.append("Aggressor side unavailable for at least one row; directional score is limited.")
        ...
        if ambiguity_flags and abs(directional_score) < 0.25:
            ...
            signal = "call_pressure" if ratio_vol is not None and ratio_vol >= 1.0 else "bullish_flow"
        elif directional_score < -0.25:
            signal = "put_pressure" if ratio_vol is not None and ratio_vol <= 1.0 else "bearish_flow"
        ...
        return OptionsFlowSignal(
            ...
            max_oi_expiry=max(expiry_oi, key=lambda expiry: expiry_oi[expiry]) if expiry_oi else None,
    ...
    def _moneyness_pressure(self: OptionsFlowSignalEngine, events: list[OptionFlowEvent]) -> dict[str, float]:
        pressure: dict[str, float] = defaultdict(float)
        for event in events:
            key = f"{event.moneyness}_{event.right}"
            pressure[key] += event.premium
        return {key: round(value, 8) for key, value in sorted(pressure.items()) if key != "unknown_unknown"}

    def _strike_clusters(self: OptionsFlowSignalEngine, events: list[OptionFlowEvent]) -> list[dict[str, float]]:
    ...
    def _ambiguity_flags(self: OptionsFlowSignalEngine, events: list[OptionFlowEvent]) -> list[str]:
        ...
        for trade_id, legs in trades.items():
            ...
            if len(legs) > 1 and {"call", "put"} <= rights:
                flags.append(f"Trade {trade_id} looks spread/hedging-like; avoid single-leg directional read.")
        return flags
...
def _optional_int(value: object) -> int | None:
    if value is None:
        return None
    try:
        return int(value) if isinstance(value, SupportsInt | str | bytes | bytearray) else None
    except (TypeError, ValueError):
        return None
