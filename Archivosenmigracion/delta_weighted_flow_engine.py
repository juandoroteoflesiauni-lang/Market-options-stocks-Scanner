...
class DeltaWeightedFlow_Engine:
    ...
    _REQUIRED_COLS: frozenset[str] = frozenset(
        ["strike", "type", "volume", "mark_price", "delta"]
    )
    ...
    def process_historical(
        self, snapshots: list[pd.DataFrame]
    ) -> list[FlowSnapshot]:
    ...
    def _validate_dataframe(self, df: pd.DataFrame) -> None:
        ...
        if missing:
            raise ValueError(
                f"option_chain is missing required columns: {sorted(missing)}"
            )
        if df.empty:
...
if __name__ == "__main__":
    ...
    if len(finite):
        print(f"   μ = {finite.mean():.4f}  |  σ = {finite.std(ddof=1):.4f}"
