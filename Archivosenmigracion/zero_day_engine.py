...
class ZeroDayEngine:
    ...
    def _prepare_chain(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "vanna" not in df.columns:
            df["vanna"] = (df.get("vega", df["gamma"] * self.spot * 0.01) / (self.spot * df["IV"].clip(0.01)))
    ...
    def _compute_rvol(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        return df[
            ["strike", "option_type", "volume", "open_interest", "rvol", "rvol_flag"]
        ].copy()
        ...
        if abs(charm_p) > charm_threshold and self.minutes_to_close < 120:
            ...
            alerts.append(
                SqueezeAlert(
                    ...
                    metadata={"charm_flow_mm": float(charm_p), "minutes_to_close": self.minutes_to_close},
                )
            )
...
def compute_zero_day_payload(
    ...
    try:
        ...
        return {
            ...
            "zone": zone,
            "gex_bars": [{"strike": float(s), "gex_bn": float(g)} for s, g in zip(strikes, gex_vals, strict=False)],
            "pin_curve": [{"strike": float(s), "pin_prob": float(p)} for s, p in zip(strikes, pin_aligned, strict=False)],
            "alerts": alerts_out,
        }
