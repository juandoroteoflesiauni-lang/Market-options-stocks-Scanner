...
class _EventDrivenLSTMCell(_nn.Module):
    ...
    def __init__(self, input_height: int, input_width: int, hidden_channels: int, event_dim: int, kernel_size: int) -> None:
        super().__init__()
        ...
        self.h_proj = _nn.Conv2d(hidden_channels, hidden_channels, kernel_size=kernel_size, padding=padding)
    ...
    def forward(self, x_t: _torch.Tensor, h_prev: _torch.Tensor, c_prev: _torch.Tensor, e_t: _torch.Tensor) -> tuple[_torch.Tensor, _torch.Tensor]:
        x_2d = x_t.unsqueeze(1)
        base = self.x_proj(x_2d) + self.h_proj(h_prev) + self.e_proj(e_t).unsqueeze(-1).unsqueeze(-1).expand(-1, -1, self.input_height, self.input_width)
        c_tilde, f_t, i_t = _torch.tanh(self.candidate_gate(base)), _torch.sigmoid(self.forget_gate(base)), _torch.sigmoid(self.input_gate(base))
    ...
    def init_hidden(self, batch_size: int, device: _torch.device) -> tuple[_torch.Tensor, _torch.Tensor]:
...
class _EventDrivenLSTM(_nn.Module):
    def __init__(self, input_height: int, input_width: int, hidden_channels: int, event_dim: int, n_layers: int, n_classes: int, dropout: float, kernel_size: int) -> None:
        super().__init__()
        self.cells: _nn.ModuleList = _nn.ModuleList(
            [_EventDrivenLSTMCell(input_height, input_width, hidden_channels, event_dim, kernel_size) for _ in range(n_layers)]
        )
        self.dropout_layer, self.classifier = _nn.Dropout(dropout), _nn.Linear(hidden_channels, n_classes)
    ...
    def forward(self, x: _torch.Tensor, e: _torch.Tensor) -> _torch.Tensor:
        ...
        return cast(_torch.Tensor, self.classifier(self.dropout_layer(h_states[-1].mean(dim=(2, 3)))))
...
class MultimodalPredictiveEngine:
    ...
    @staticmethod
    def prepare_batch(df_fund: pd.DataFrame, df_news: pd.DataFrame, ticker: str, config: MultimodalModelConfig) -> MultimodalBatch | None:
        """Aligns data and builds fusion tensors."""
        try:
            # 1. Alignment & Preprocessing
            aligned_fund, aligned_news = df_fund.tail(config.sequence_length), df_news.tail(config.sequence_length)
            ...
            return MultimodalBatch(
                ticker=ticker, sequence_length=config.sequence_length, prediction_horizon=config.prediction_horizon,
                tensor_sequence=np.expand_dims(tensor_3d, 0), event_sequence=np.random.rand(1, config.sequence_length, 1),
    ...
    async def run_fusion_inference_async(self, symbol: str, ohlcv_df: pd.DataFrame, sentiment_score: float = 0.0, gex_data: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            dl_task = loop.run_in_executor(None, self.ai_engine.analyze, symbol, ohlcv_df, sentiment_score)
            vsa_task = loop.run_in_executor(None, self.vsa_forecast.predict_current_bar, ohlcv_df, 300, 3600, symbol)
            ...
            return {
                "symbol": symbol, "bias": dl_result.signal, "conviction": fusion_score,
                "fusion_metadata": {
    ...
    def run_fusion_inference(self, symbol: str, ohlcv_df: pd.DataFrame, sentiment_score: float = 0.0, gex_data: dict[str, Any] | None = None) -> dict[str, Any]:
        """High-level Orchestrator Facade (VSA + DL + GEX)."""
        try:
            # 1. DL Inference (New institutional kernel)
            dl_result = self.ai_engine.analyze(ticker=symbol, df_fundamentals=ohlcv_df, sentiment_score=sentiment_score)
