...
class SqueezeIgnitionEngine:
    ...
    def _transition(
        ...
        if self._state == SqueezeState.VULNERABLE:
            ...
            ignition_conditions_met = (
                svs >= SVS_IGNITION_THRESHOLD
                and wall_crossed
                and vol_accel
            )
...
if __name__ == "__main__":
    ...
    assert signal_p4.signal_type == SignalType.LONG_MOMENTUM_IGNITION, \
    ...
    assert signal_p4.suggested_entry is not None, \
        "✗ Precio de entrada sugerido ausente"
    print(f"  ✓ Precio de entrada sugerido: ${signal_p4.suggested_entry:.2f}")

    assert len(signal_p4.take_profit_levels) > 0, \
        "✗ Niveles de TP ausentes"
    print(f"  ✓ Niveles de Take Profit: {signal_p4.take_profit_levels}")

    assert engine.state == SqueezeState.MONITORING, \
        f"✗ Estado final inesperado: {engine.state}"
    print(f"  ✓ Motor volvió a MONITORING tras el periodo de COOLING")
